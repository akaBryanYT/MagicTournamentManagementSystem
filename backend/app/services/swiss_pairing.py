"""
Swiss pairing service for the Tournament Management System.
"""

class SwissPairingService:
    """Service for Swiss pairing algorithm."""
    
    def create_pairings(self, player_ids, previous_matches):
        """
        Create pairings for the next round using Swiss algorithm.
        
        Args:
            player_ids: List of player IDs sorted by standings
        ...
        if len(player_ids) % 2 != 0:
            # Find player who hasn't had a BYE yet
            has_had_bye = set()
            for match in previous_matches:
